...
<div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-1">Create and manage course assignments</p>
    </div>
    <button onclick="document.getElementById('createModal').classList.remove('hidden')" class="px-4 py-2.5 bg-terracotta text-white rounded-xl text-sm font-semibold hover:opacity-90 transition-all flex items-center justify-center gap-2 shadow-lg shadow-terracotta/30 hover:-translate-y-0.5">
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
    <div class="bg-white rounded-2xl border border-gray-100 shadow-md shadow-terracotta/5 p-6 flex items-center gap-4 transition-all hover:shadow-lg hover:border-terracotta/10">
        <div class="w-12 h-12 rounded-xl bg-terracotta/10 text-terracotta flex items-center justify-center shrink-0">
        ...
        <div>
            ...
            <h3 class="text-2xl font-bold text-gray-900">{{ collect($assignments)->count() }}</h3>
        </div>
    </div>
    <div class="bg-white rounded-2xl border border-gray-100 shadow-md shadow-indigo-500/5 p-6 flex items-center gap-4 transition-all hover:shadow-lg hover:border-blue-100">
        ...
        <div>
            <p class="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1">Submissions</p>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-md shadow-indigo-500/5 p-6 flex items-center gap-4 transition-all hover:shadow-lg hover:border-emerald-100">
        ...
        <div>
            <p class="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1">Graded</p>
            <h3 class="text-2xl font-bold text-gray-900">0</h3>
        </div>
    </div>
    <div class="bg-white rounded-2xl border border-gray-100 shadow-md shadow-indigo-500/5 p-6 flex items-center gap-4 transition-all hover:shadow-lg hover:border-amber-100">
        ...
        <div>
            <p class="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1">Pending</p>
            <h3 class="text-2xl font-bold text-gray-900">0</h3>
        </div>
    </div>
</div>
...
<div x-data="{ searchQuery: '', selectedCourse: '' }" class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-12 shadow-indigo-500/5">
    <div class="p-5 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gray-50/30">
        <h3 class="font-bold text-gray-900">All Assignments</h3>
        <div class="flex items-center gap-3">
            <div class="relative">
                ...
                <input type="text" x-model="searchQuery" placeholder="Search assignments..." class="pl-9 pr-4 py-2 text-sm bg-white border border-gray-200 rounded-lg focus:outline-none focus:border-terracotta focus:ring-1 focus:ring-terracotta transition w-full md:w-64">
            </div>
            <select x-model="selectedCourse" class="px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm text-gray-600 focus:outline-none focus:border-terracotta transition appearance-none cursor-pointer hidden md:block">
        ...
        <table class="w-full text-left border-collapse min-w-[700px]">
            <thead>
                <tr class="bg-white border-b border-gray-100 text-[11px] text-gray-400 font-bold uppercase tracking-wider">
                    <th class="p-4 pl-6">Assignment Title</th>
                    <th class="p-4">Associated Course</th>
                    <th class="p-4 text-center">Total Points</th>
            ...
            <tbody class="divide-y divide-gray-50 bg-white">
                @forelse($assignments as $assignment)
                    <tr x-show="(searchQuery === '' || '{{ strtolower($assignment->lesson->title ?? '') }}'.includes(searchQuery.toLowerCase())) && (selectedCourse === '' || '{{ $assignment->lesson->module->course->id ?? '' }}' === selectedCourse)" class="group hover:bg-[#F9F9FF] transition-colors relative">
                        <td class="p-4 pl-6 font-semibold text-gray-900">
                            <!-- Left highlight border on hover -->
                            <div class="absolute left-0 top-0 bottom-0 w-1 bg-terracotta opacity-0 group-hover:opacity-100 transition-opacity"></div>
                            {{ $assignment->lesson->title ?? 'Unknown Lesson' }}
                        </td>
                        <td class="p-4 text-sm text-gray-500 font-medium">{{ $assignment->lesson->module->course->title ?? 'N/A' }}</td>
                        <td class="p-4 text-sm font-bold text-gray-900 text-center">{{ $assignment->total_marks }}</td>
                        ...
                        <td class="p-4 pr-6 text-right relative">
                            ...
                            <div class="flex justify-end relative" x-data="{ open: false }" @click.outside="open = false">
                                <button @click="open = !open" class="text-gray-400 hover:text-terracotta p-1.5 rounded-lg hover:bg-white border border-transparent hover:border-gray-200 transition shadow-sm hover:shadow-md">
                                ...
                                <div x-show="open" class="absolute right-0 top-10 w-48 bg-white rounded-xl shadow-lg shadow-indigo-500/10 border border-gray-100 z-10 py-1" style="display: none;" x-transition.opacity.duration.200ms>
                                    @if($assignment->lesson && $assignment->lesson->module && $assignment->lesson->module->course)
                                        <a href="{{ route('instructor.course.builder', $assignment->lesson->module->course->id) }}" class="flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-50 hover:text-[#5A4BFF] transition">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="p-16 text-center">
                            ...
                            <h4 class="text-xl font-bold text-gray-900 mb-2">No Assignments Yet</h4>
                            <p class="text-gray-500 text-sm mb-8 max-w-sm mx-auto leading-relaxed">Create interactive assignments to test your students' knowledge and track their progress through your courses.</p>
...
<!-- Smart Modal for Creating Assignment -->
    ...
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100">
        <div class="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
            <h3 class="text-lg font-bold text-gray-900 flex items-center gap-2">
                <svg class="w-5 h-5 text-terracotta" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
        ...
        <div class="p-6">
            <p class="text-sm text-gray-500 mb-6 leading-relaxed">Select the course you want to add an assignment to. You will be redirected to its powerful curriculum builder.</p>
            ...
            <div class="flex items-center justify-end gap-3 mt-8 pt-4 border-t border-gray-100">
                ...
                <button onclick="if(document.getElementById('courseSelect').value){ window.location.href='/instructor/course/' + document.getElementById('courseSelect').value + '/builder'; }" class="px-6 py-2.5 bg-terracotta text-white text-sm font-bold rounded-xl hover:opacity-90 transition shadow-lg shadow-terracotta/30 flex items-center gap-2">
                    Continue to Builder
